...
PYTHONPATH=. deepspeed --master_port=29501 --num_gpus=$num_gpus train/mm_tune.py \
    --run_id "surg390k_qwen2_5-3b-vl" \
    --model Qwen/Qwen2.5-VL-3B-Instruct \
    --datasets $datastore/surg_390k/surg_390k_train.jsonl \
    --sample_size 100000 \
    --num_train_epochs 3 \
    ...
    --max_length 8192 \
    --run_seed $run_seed \
    --run_output_dir "${OUTPUT_DIR}/surg390k/" \
    --save_n_epochs 1 \
    --dataset_num_proc 6 \
    --skip_eval \
    --deepspeed "$ds_config"
