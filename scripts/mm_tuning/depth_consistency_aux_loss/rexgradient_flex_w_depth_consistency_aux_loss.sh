...
model_store=experts
datastore=traces/multimodal/rexgradient
ds_config=train/ds_config/v0.json
run_seed=42
num_gpus=4

PYTHONPATH=. torchrun --master_port=29501 --nproc_per_node=$num_gpus train/mm_tune.py \
    --run_id "rexgradient_qwen2_5-3b-vl-flex-router_hidden_state_init-depth_consistency_loss" \
    --model micdun/rexgradient_hidden_state_init \
    --datasets "$datastore/rexgradient_train_w_length.jsonl" \
    --train_expert_idx 1 \
    --num_train_epochs 1 \
    ...
    --router_depth_aux_loss_coef 0.001
