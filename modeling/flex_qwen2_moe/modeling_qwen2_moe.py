...
class Qwen2MoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        self.normalize_router_gate_and_hidden = config.normalize_router_gate_and_hidden
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.normalize_router_gate_and_hidden:
            hidden_for_router = F.normalize(hidden_states, p=2, dim=-1)
            gate_for_router = F.normalize(self.gate.weight, p=2, dim=-1)
        else:
            hidden_for_router = hidden_states
            gate_for_router = self.gate.weight
        router_logits = hidden_for_router @ gate_for_router.T  # (batch*seq, n_experts)
        ...
        # No longer need to interpolate shared expert
