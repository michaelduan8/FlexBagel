...
def parse_option():
    ...
    parser.add_argument('--pred', type=str, default="test.json", help='path to prediction + groundtruth file', )
    args, unparsed = parser.parse_known_args()
    return args


def evaluate(pred, quilt=False):    
    ...
    for item in pred:
        ...
        if item['answer_type'] == 'OPEN' or item['answer_type'] == 'other':
            ...
            closed_scores2['q_id'].append(item['id'])

            if quilt:
                gt_value = item['yes_no_answer'].lower()

            assert gt_value in ['yes', 'no'], f"assert gt_value in : {item['id'], gt_value}"
            answer = gt_value
...
if __name__ == '__main__':
    ...
    # perform evaluation
    results = evaluate(pred, quilt=args.quilt)
